...
def vf_profile_at_target_j(vf, jf, weights, J_target, dJ_window, v_edges, dv):
    """
    Build dσ/dv_f for a selected target j_f = J_target by summing over
    jf in [J_target - dJ_window/2, J_target + dJ_window/2).
    """
    lo = J_target - 0.5 * dJ_window
    hi = J_target + 0.5 * dJ_window
    ...
    hist, _ = np.histogram(vf_sel, bins=v_edges, weights=w_sel)
    dsigma_dv = hist / dv

    return dsigma_dv, mask.sum(), lo, hi
...
def write_vf_profiles_for_selected_j(root, vf, jf, weights, v_edges, v_cent, dv,
                                     J_targets, dJ_window):
    manifest = f"{root}.vf_profiles_manifest.txt"
    with open(manifest, "w") as mf:
        mf.write("# target_jf  lo  hi  ntraj  filename\n")

        for J in J_targets:
            prof, ntraj, lo, hi = vf_profile_at_target_j(
                vf, jf, weights,
                J_target=J,
                dJ_window=dJ_window,
                v_edges=v_edges,
            ...
            tag = str(J).replace("-", "m").replace(".", "p")
            win_tag = str(dJ_window).replace("-", "m").replace(".", "p")
            outfile = f"{root}.vf_profile_j{tag}_win{win_tag}.dat"

            with open(outfile, "w") as f:
                f.write(f"# target_jf = {J:.6f}\n")
                f.write(f"# window = [{lo:.6f}, {hi:.6f})\n")
                f.write(f"# ntraj = {ntraj}\n")
                f.write("# vf dsigma_dv\n")
                for v, y in zip(v_cent, prof):
                    if y <= 0.0:
                        continue
                    f.write(f"{v:12.6f} {y:16.8e}\n")

            mf.write(f"{J:12.6f} {lo:12.6f} {hi:12.6f} {ntraj:10d} {outfile}\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python classical_diff_cs.py file.big [dv] [dj] [weight_mode] [dJ_window] [J_targets]")
        print("")
        print("Examples:")
        print("  python classical_diff_cs.py e2627_v0j18.big 0.1 0.1 uniform_b 2 0,2,6,10,18")
        print("  python classical_diff_cs.py e2627_v0j18.big 0.1 0.1 uniform_b2 1 0:20:2")
        return
    ...
    dJ_window = float(sys.argv[5]) if len(sys.argv) > 5 else 1.0
    J_targets = parse_j_targets(sys.argv[6]) if len(sys.argv) > 6 else [0, 2, 4, 6, 8, 10]
    ...
    print("dJ_window:", dJ_window)
    print("J_targets:", J_targets)
    ...
    manifest = write_vf_profiles_for_selected_j(
        root=root,
        vf=vf2,
        jf=jf2,
        weights=w2,
    ...
    print("  selected-j manifest    :", manifest)
    print("")
